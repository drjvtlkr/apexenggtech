...
const Navbar = () => {
  ...
  const [isShrunk, setIsShrunk] = useState(false); // To track if the image is fully shrunk
  const pathname = usePathname();
  const isActive = (path: string) => pathname === path;

  useEffect(() => {
    const handleScroll = () => {
      const scrollY = Math.min(window.scrollY, 100); // Cap the scroll effect at 100px
      const progress = Math.max(1 - scrollY / 100, 0); // Scale from 1 to 0
  ...
  return (
    <>
      <nav className="fixed top-0 w-full bg-transparent backdrop-blur-sm text-black flex items-center justify-between p-2 transition-all duration-300 shadow-md">
